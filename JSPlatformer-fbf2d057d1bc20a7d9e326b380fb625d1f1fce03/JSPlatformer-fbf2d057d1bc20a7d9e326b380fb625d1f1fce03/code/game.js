...
var actorChars = {
  "@": Player,
  "c": Crystal, // A coin will wobble up and down
  "v": Lava,
  "e": Enemy,
  "f": Falling,
  "p": Portal
...
function Falling(pos) {
  this.pos = pos;
  this.size = new Vector(0.9, 0.9);
  this.speed = new Vector(0, 3);
  this.repeatPos = pos;
}
Falling.prototype.type = "falling";

function Portal(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.9, 0.9);
}
Portal.prototype.type = "portalOff";
...
var wobbleESpeed = .5, wobbleEDist = 11;
  ...
  if (type == "lava" && this.status == null) {
    ...
    // If there aren't any coins left, player wins
     if (!this.actors.some(function(actor) {
		...
		Portal.prototype.type = "portalOn";
    }
  }
  if(type == "portalOn" && this.status == null) {
	  this.status = "won";
	  this.finishDelay = 1;
  }
};
